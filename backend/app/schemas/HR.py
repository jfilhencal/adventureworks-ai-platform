...
class HRSummary(BaseModel):
    ...
    department_distribution: dict[str, int] = Field(..., description="Employee count per department")
...
class MonthlyHRDataPoint(BaseModel):
    ...
    cumulative_headcount: int = Field(..., description="Cumulative headcount at the end of the month")


class AIAnalysisRequest(BaseModel):
    """Request payload for AI analysis."""
    prompt: str = Field(..., min_length=3, description="User prompt for AI analysis")
